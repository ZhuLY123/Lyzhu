function itheta=NiJie(T_goat)
    ...
    T67=[cos(theta7),-sin(theta7),0,a6;sin(theta7)*cos(afa6),cos(theta7)*cos(afa6),-sin(afa6),-sin(afa6)*d7;
        sin(theta7)*sin(afa6),cos(theta7)*sin(afa6),cos(afa6),cos(afa6)*d7;0,0,0,1];
    T06=T_goat/T67;
    %将T06各元素提取出来赋值
    nx=T06(1,1);
    ny=T06(2,1);
    ...
    if ForJudgment<-1e-6
        ...
        if abs(S5_2)>1e-6
            ...
            B1(2)=cos(theta1_1)*px+sin(theta1_1)*py-d5*S234(2);B2(2)=pz-d1+d5*C234(2);A(2)=-2*B2(2)*a2;
            B(2)=2*B1(2)*a2;C(2)=B1(2)^2+B2(2)^2+a2^2-a3^2;
